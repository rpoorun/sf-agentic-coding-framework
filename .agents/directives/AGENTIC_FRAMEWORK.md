...
| gstack | https://github.com/garrytan/gstack | multiple skill folders at repo root plus `docs/skills.md`; MIT licensed | **Not Salesforce-specific.** General-purpose agent workflow methods for requirement discovery, plan review, code review, debugging, QA, documentation, diagrams, handoff, safety, and skill evaluation. High-fit methods are synthesized into the `agentic-*` skill family and selected workflow/directive updates. Runtime-coupled browser, iOS, setup, sync, and generated-template content is excluded. |
...
| apex-consts | https://github.com/beyond-the-cloud-dev/apex-consts | classes under `force-app/main/default/classes/` (`Consts.cls` root + `concrete-consts/{SObject}Consts.cls`) | **Salesforce-native, MIT licensed.** Lazy-singleton constants pattern (one `{SObject}Constants` class per object, exposed via `Constants.{OBJECT}`) replacing hardcoded picklist/API-name literals. Adapted — not copied verbatim — into `sf-platform-apex/assets/Constants.cls` and `assets/concrete-constants/AccountConstants.cls`, renaming every class from the upstream `Consts` abbreviation to the full word `Constants` per this project's naming standard. Mandated by [APEX_CONSTANTS_FRAMEWORK.md](../standards/APEX_CONSTANTS_FRAMEWORK.md). |
...
sfdc-trigger-framework and apex-consts are the opposite case: they are already small, dependency-free, Salesforce-native code, so they are vendored as `.cls` template assets under `sf-platform-apex/assets/` rather than refactored into prose — `TriggerHandler.cls` is copied verbatim; the apex-consts classes are copied structurally but every class name is renamed from the upstream `Consts` abbreviation to the full word `Constants` (`Constants.cls`, `AccountConstants.cls`) per this project's no-abbreviated-class-names standard. When this framework is (re-)installed or refreshed, re-fetch both repositories and diff the vendored classes against the latest upstream version — update `assets/TriggerHandler.cls`, `assets/TriggerHandlerTest.cls`, and `assets/Constants.cls` if upstream has changed (re-applying the `Consts` → `Constants` rename to any new upstream class), following the same approval-before-merge discipline as [Master Framework Repository And Sync Workflow](#master-framework-repository-and-sync-workflow), and update [APEX_TRIGGER_FRAMEWORK.md](../standards/APEX_TRIGGER_FRAMEWORK.md) / [APEX_CONSTANTS_FRAMEWORK.md](../standards/APEX_CONSTANTS_FRAMEWORK.md) if the usage pattern itself changed upstream.
...
### Generic Agentic Skill Synthesis

General-purpose, non-Salesforce workflow skills use the `agentic-{name}` convention documented in [AGENTIC_SKILLS.md](../skills/AGENTIC_SKILLS.md). They are installed at `{USER_AGENTS}/skills/` like Salesforce skills, but they do not use the `sf-{cloud}-{name}` prefix because they apply across repositories and technologies.

When adding or updating an `agentic-*` skill:

1. Fetch or inspect the upstream method from its source repository; do not invent source-derived behavior from memory.
2. Classify whether the method belongs in a new `agentic-*` skill, an existing `agentic-*` skill, an existing `sf-*` skill, a workflow, a standard, or a directive.
3. Synthesize methodology into framework-native language. Do not copy raw slash-command wrappers, host-specific setup steps, generated `.tmpl` files, daemons, browser binaries, or tool-specific runtime code.
4. Attribute upstream sources in `metadata.sources`, [AGENTIC_SKILLS.md](../skills/AGENTIC_SKILLS.md), [README.md](../../README.md), and [CHANGELOG.md](../CHANGELOG.md) when user-visible.
5. Keep each skill generic, parameterized, and self-contained. Project-specific examples belong in project-level helpers or documentation.
6. Add router entries in [AGENTIC_SKILLS.md](../skills/AGENTIC_SKILLS.md), and add cross-routing in [SALESFORCE_SKILLS.md](../skills/SALESFORCE_SKILLS.md) only where the generic workflow strengthens Salesforce delivery.

For gstack specifically, exclude `ios-*`, `open-gstack-browser`, `setup-browser-cookies`, `pair-agent`, `setup-gbrain`, `sync-gbrain`, `gstack-upgrade`, `setup-deploy`, `scrape`, `skillify`, browser-skill internals, Bun build tooling, generated templates, and any runtime files. Adapt only the reusable methods behind high-fit skills.
...
1. Track the last check in the user-level preferences file, `{USER_AGENTS}/preferences.json` (where `{USER_AGENTS}` is `~/.agents/` on Unix or `%USERPROFILE%\.agents\` on Windows). Read the `last_checked_utc` field (ISO 8601 UTC datetime, e.g. `"2026-07-01T08:00:00Z"`). If the file does not yet exist and `{USER_AGENTS}` is writable, run [Step 0a of PROJECT_BOOTSTRAP.md](../workflows/PROJECT_BOOTSTRAP.md#step-0a--initialise-user-level-framework-directory) to create the user-level framework directory. If `{USER_AGENTS}` is **not writable** (sandboxed/remote agent), do not attempt to create or write `preferences.json` at all — track the check with an in-session marker only (run the version check once per session, then remember in-session that it ran). This file lives outside any repo and is never committed.
...
4. Compare the master's version to the local install's `AGENTS.md` "Version" field. If `{USER_AGENTS}/preferences.json` is writable, update it — set `last_checked_utc` to the current UTC datetime and `last_known_version` to the master version just read — regardless of the outcome, so the check does not repeat later the same day. If it is not writable, rely on the in-session marker from step 1 and skip the file write; never treat a failed marker write as a failed update check.
