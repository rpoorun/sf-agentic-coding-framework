...
1. Track the last check in the user-level preferences file, `{USER_AGENTS}/preferences.json` (where `{USER_AGENTS}` is `~/.agents/` on Unix or `%USERPROFILE%\.agents\` on Windows). Read the `last_checked_utc` field (ISO 8601 UTC datetime, e.g. `"2026-07-01T08:00:00Z"`). If the file does not yet exist and `{USER_AGENTS}` is writable, run [Step 0a of PROJECT_BOOTSTRAP.md](../workflows/PROJECT_BOOTSTRAP.md#step-0a--initialise-user-level-framework-directory) to create the user-level framework directory. If `{USER_AGENTS}` is not writable (sandboxed/remote agent), skip the persistent marker and run the version check unconditionally once per session. This file lives outside any repo and is never committed.
