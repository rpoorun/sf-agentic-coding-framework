...
- **Secondary store**: [`.agents/project/ENVIRONMENT.md`](../project/ENVIRONMENT.md#author-identity) — team-shared fallback when the framework is committed to a project repo.
- Resolution order: if `{USER_AGENTS}/identity.json` has a non-empty `author_name`, use it. Otherwise fall back to `ENVIRONMENT.md`. If both are blank, **stop and ask the user** for their author name and email before generating the first Apex/LWC class or method comment block in the session. Ask once per missing value; do not re-ask within the same session once answered.
- After the user answers, ask whether to persist it to `{USER_AGENTS}/identity.json` (user-level, shared across repos) or `ENVIRONMENT.md` (team-shared, committed to this repo) for future sessions.
