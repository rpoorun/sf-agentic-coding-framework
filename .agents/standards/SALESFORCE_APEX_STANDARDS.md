...
- **Secondary store**: `{REPO_STATE}/project/ENVIRONMENT.md` (Author Identity section; template at [`.agents/project/ENVIRONMENT.md`](../project/ENVIRONMENT.md#author-identity)) — per-repo override when a specific project needs a different identity (e.g. client-specific email).
- Resolution order: if `{USER_AGENTS}/identity.json` has a non-empty `author_name`, use it. Otherwise fall back to the user-level per-repo `ENVIRONMENT.md`. If both are blank, **stop and ask the user** for their author name and email before generating the first Apex/LWC class or method comment block in the session. Ask once per missing value; do not re-ask within the same session once answered.
- After the user answers, ask whether to persist it to `{USER_AGENTS}/identity.json` (user-level, shared across repos) or `{REPO_STATE}/project/ENVIRONMENT.md` (this repo only) for future sessions.
