...
The `@author` / `@last modified by` value must be the project's configured human author identity (name and/or email), never a placeholder and never an AI/model/tool name — see [Author Identity](#author-identity-required) below. If `{USER_AGENTS}/identity.json` does not yet have an author name/email recorded (or falls back to `{PROJECT_AGENTS}/project/ENVIRONMENT.md` and that is also blank), stop and ask the user for it before generating the first class/method header in the session; do not invent one and do not write "OpenAI", "Anthropic", "Claude", "ChatGPT", "AI", "Generated by AI/assistant", or any similar attribution.
...
- **Primary store**: `{USER_AGENTS}/identity.json` (`author_name`, `author_email`) — user-level, shared across all repos on this machine. Check here first.
- **Secondary store**: `{PROJECT_AGENTS}/project/ENVIRONMENT.md` (Author Identity section; template at [`.agents/documentation/ENVIRONMENT.md`](../documentation/ENVIRONMENT.md#author-identity)) — per-project override when a specific project needs a different identity (e.g. client-specific email).
- Resolution order: if `{USER_AGENTS}/identity.json` has a non-empty `author_name`, use it. Otherwise fall back to the project-level `ENVIRONMENT.md`. If both are blank, **stop and ask the user** for their author name and email before generating the first Apex/LWC class or method comment block in the session. Ask once per missing value; do not re-ask within the same session once answered.
- After the user answers, ask whether to persist it to `{USER_AGENTS}/identity.json` (user-level, shared across repos) or `{PROJECT_AGENTS}/project/ENVIRONMENT.md` (this repo only) for future sessions.
