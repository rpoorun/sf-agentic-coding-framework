...
- **Standing migration check on every update install**: old-structure detection is no longer tied to the release that introduced the user-level architecture. On every framework update, if the current branch, another local branch, or an isolated worktree still follows the old repo-level structure (framework folders in `.agents/`, repo-level `.local-config.json` credentials, repo-level tickets/board), the update installs normally and a **remnant migration** runs: only content newer than the existing user-level state is merged in, common instruction files stay authoritative at `{USER_AGENTS}/`, and the old files are cleaned off the current branch/worktree — stale branches can never silently reintroduce the old architecture or orphan their data.
- **Merge-not-override migration semantics**: migrating instructions never overwrites or deletes an existing user-level file — differing copies are diffed and merged to complement each other (user-level base + additive repo-level guidance folded in; project-specific tailoring routed to `{REPO_STATE}/directives|workflows/`). Credentials migrate at the **connection level**: the union of all connector keys and non-empty fields across every `.local-config.json` copy found, with same-field conflicts resolved by the user (values never echoed) and each migrated connection verified (e.g. Jira auth test) before any source copy is deleted — no existing connection is ever lost.
