...
4. Either way, also add `.agents/.update-check` to `.gitignore` (regardless of the shared/local-only answer) — it is the operational marker file for the [Daily Update Check](../directives/AGENTIC_FRAMEWORK.md#daily-update-check-automatic), not framework content, and must never be committed even when the rest of the framework is shared.
