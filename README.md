...
3. **When installed into a Salesforce project**, the framework uses a three-tier architecture: you copy only `AGENTS.md` into the project repo. On first use, the agent clones the framework into `~/.agents/` (or `%USERPROFILE%\.agents\` on Windows) — the **user level**, shared across all projects on your machine. Project state (credentials, per-environment config, tickets, board, skill helpers) lives at the **project level**, `~/.agents/{PROJECT_NAME}/` (the project name comes from the git remote's repo name), shared by every repo, branch, and worktree of that project. The **repo level** keeps only `AGENTS.md` and team-shared `.agents/` content (documentation, optional repo-specific workflows/skills), committed via git. Transient data goes to the OS temp directory (`%TEMP%`/`$TMPDIR`) so the platform cleans it up. Sandboxed or remote-hosted agents that cannot write to the user level resolve framework files, credentials, state, and temp locations through fallback chains instead — see the Layered Resolution section in [`AGENTS.md`](AGENTS.md).
