...
3. **When installed into a Salesforce project**, the framework uses a user-level architecture: you copy only `AGENTS.md` into the project repo. On first use, the agent clones the framework into `~/.agents/` (or `%USERPROFILE%\.agents\` on Windows) — shared across all repos on your machine. Per-repo state (credentials, tickets, board) lives at `~/.agents/{repo_name}/`, persisting across branches and sessions. The project repo itself keeps only `AGENTS.md` and `.agents/project/` (team-shared docs).
...
2. Open the project with your AI coding agent. On first read, the agent detects `{USER_AGENTS}/` does not exist and runs the [Project Bootstrap](.agents/workflows/PROJECT_BOOTSTRAP.md): it clones the framework into `~/.agents/` (one-time per machine), creates per-repo state at `~/.agents/{repo_name}/`, and interviews you for your dev org, Git remote, and team setup.
